...
module.exports = (function() {
    ...
    Workbook.prototype.replaceAll = function(str, find, replace) {
        return str.replace(new RegExp(escapeRegExp(find), 'g'), replace);
    }

    Workbook.prototype.insertFormularCell = function(cell, substitution) {
        cell.attrib.t = "e"

        var value = substitution.formular
        for (var key in substitution.eval) {
            
            var data = substitution
            data.row = cell.attrib.r.match(/\d+/)[0];

            var exp = substitution.eval[key]
            value = this.replaceAll(value, key, eval(exp))
        }

        var newElement = new etree.Element("f")
        newElement.text = value;
        cell._children = [newElement]
    ...
    Workbook.prototype.insertCellValue = function(cell, substitution) {
        ...
        if (substitution instanceof Object && substitution.formular != null) {
            return this.insertFormularCell(cell, substitution)
        }
